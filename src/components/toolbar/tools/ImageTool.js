...
class ImageTool extends MakeTool {
    ...
    applySyntax(event) {
        ...
        if(this.fileInputConfig){
            fileInputTag = `
                <input type="file" accept="${acceptFormats}" class="fj:input fj:w-full img-file-input">
            ...
            const bodyHTML =`
                ...
                <form method="post">
                    <div class="fj:flex fj:flex-col fj:justify-center fj:gap-y-4.5 fj:mt-4">
                        ${fileInputTag}
                        <input type="url" placeholder="URL" class="fj:input fj:w-full img-link-input" required>
                        <input type="text" placeholder="Alt text" class="fj:input fj:w-full img-link-alt-input" value="${selectedText}" ${(altRequired) && 'required'}>
                        <button type="submit" class="fj:btn fj:btn-sm submit-img-link fj:self-end">Apply</button>
                    </div>
                </form>`;
