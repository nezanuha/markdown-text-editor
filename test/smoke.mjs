...
const FULL_BAR = ['heading', 'bold', 'italic', 'ul', 'ol', 'checklist', 'blockquote',
                  'code', 'codeblock', 'hr', 'table', 'link', 'image',
                  'undo', 'redo', 'indent', 'outdent', 'variables', 'preview'];
...
check('every tool constructs without throwing', () => {
    const e = makeEditor({ toolbar: FULL_BAR, variables: VARS });
...
check('malformed variables do not throw and keep the good entry', () => {
    const e = makeEditor({
        toolbar: FULL_BAR,
        variables: [null, 'garbage', { label: 'No value' }, { label: 'Empty', items: [] },
                    { label: 'Works', value: '{{ok}}' }]
    });
    return labels(e).length === 1 && labels(e)[0] === 'Works';
});

check('variables alone is enough, no toolbar option needed', () => {
    const e = makeEditor({ variables: VARS });
    return e.editorContainer.querySelector('.variable-btn') !== null;
});

check('default toolbar shows no variable button when unconfigured', () => {
    const e = makeEditor({});
    return e.editorContainer.querySelector('.variable-btn') === null;
});
...
check('clicking an entry inserts its value', () => {
    const e = makeEditor({ toolbar: FULL_BAR, variables: VARS }, 'Hi ');
...
check('labels are not treated as markup', () => {
    const e = makeEditor({ toolbar: FULL_BAR, variables: [{ label: '<img src=x onerror=alert(1)>', value: '{{x}}' }] });
